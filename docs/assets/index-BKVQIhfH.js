...
**Returning Officer**  
*(College Seal)*`},{id:`statutory_notice_booth_allotment`,title:`Notice on Polling Booths, Station Locations & Class Allotments`,refNo:`${o}/ELEC/${i}/NOTIF-02`,date:s,category:`Polling Booth Info`,pinned:!0,isPublished:!0,signatoryName:`Returning Officer`,signatoryTitle:`Returning Officer, ${a}`,content:`### POLLING BOOTHS & STATIONS ALLOTMENT NOTICE

Notice is hereby given to all students and electors of **${a}** regarding the designated **Polling Stations and Booth Allotments** for the College Union Elections ${i}.

Voting shall take place strictly at the designated polling booths between the official polling hours:
**${c(t.pollingStart,`9:30 AM`)} to ${c(t.pollingEnd,`1:30 PM`)}**.
...
*Electors are requested to verify their names on the Nominal Roll facing sheet outside their respective booth before joining the queue.*`},{id:`statutory_notice_code_of_conduct`,title:`Model Code of Conduct & Campus Campaigning Directives`,refNo:`${o}/ELEC/${i}/MCC-03`,date:s,category:`Code of Conduct`,pinned:!1,isPublished:!0,signatoryName:`Returning Officer`,signatoryTitle:`Returning Officer & Principal, ${a}`,content:`### MODEL CODE OF CONDUCT FOR CANDIDATES AND STUDENT GROUPS

With the announcement of the College Union Election ${i}, the **Model Code of Conduct (MCC)** comes into force with immediate effect across the entire campus. All contesting candidates, election agents, proposers, seconders, and students must strictly adhere to the following:
...
5. **Silence Period**:
   - All physical canvassing and campaigning shall cease 24 hours prior to the commencement of polling.`},{id:`statutory_notice_voter_instructions`,title:`Voter Guidelines & Mandatory Identity Verification Instructions`,refNo:`${o}/ELEC/${i}/VOTE-04`,date:s,category:`Voter Instructions`,pinned:!1,isPublished:!0,signatoryName:`Returning Officer`,signatoryTitle:`Returning Officer, ${a}`,content:`### GUIDELINES FOR VOTERS ON POLLING DAY

All bona fide regular students whose names appear in the Final Nominal Roll of **${a}** are requested to note the following crucial voting guidelines:
...
6. Vacate the polling station premises immediately to maintain smooth queue movement.`},{id:`statutory_notice_counting_and_results`,title:`Notice on Scrutiny, Counting of Ballots & Results Declaration`,refNo:`${o}/ELEC/${i}/COUNT-05`,date:s,category:`Counting & Results`,pinned:!1,isPublished:!0,signatoryName:`Returning Officer`,signatoryTitle:`Returning Officer, ${a}`,content:`### COUNTING OF VOTES AND DECLARATION OF RESULTS

Notice is hereby given that the scrutiny and counting of votes polled in the College Union Elections ${i} will commence at the **Central Counting Hall (Auditorium)** as per the schedule below:

- **Counting Commences**: ${c(t.resultsStart,`Immediately following closure of poll`)}
...
5. In the event of an equality of votes between two candidates, the result shall be decided by the Returning Officer drawing lots in the presence of the candidates.`}]}async function Ft(e){let t=U();t&&(W(e,`notices`,`
  ...
  `),await It(e.querySelector(`#adminMain`),t))}async function It(e,t){if(e)try{let[n,r]=await Promise.all([C.adminGetNotices(t,!0).catch(()=>({})),C.getNominalRoll().catch(()=>[])]),i=n.settings||{},a=n.schedule||{},o=Array.isArray(n.booths)?n.booths:[];Array.isArray(n.locations)&&n.locations;let s=Array.isArray(n.notices)?n.notices:[];s.length===0&&(s=Pt(i,a,o));let c={};r.forEach(e=>{let t=String(e.CLASS||``).trim(),n=String(e.Dept||``).trim(),r=t.toUpperCase().includes(`RESEARCH`)||t.toUpperCase().includes(`SCHOLAR`)||t.toUpperCase().includes(`PHD`)?`RESEARCH SCHOLAR - ${n}`:t;r&&(c[r]||(c[r]={name:r,dept:n,count:0}),c[r].count++)}),o.forEach(e=>{let t=Array.isArray(e.classes)?e.classes:[];e.totalStudents=0,t.forEach(t=>{c[t]&&(e.totalStudents+=c[t].count)})}),Lt(e,t,i,a,s,o,c)}catch(t){console.error(`Error loading admin notices:`,t),e.innerHTML=`<div class="alert alert-error">❌ ${F(t.message||`Failed to load notices`)}</div>`}}function Lt(e,t,r,i,a,o,s){r.collegeName||n.COLLEGE_NAME,r.collegeShortName||n.COLLEGE_SHORT_NAME,r.electionYear||new Date().getFullYear();let c=a.filter(e=>e.isPublished!==!1&&e.isPublished!==`false`).length,l=o.reduce((e,t)=>e+(t.totalStudents||0),0);e.innerHTML=`
    <div class="page-enter space-y-6">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="glass rounded-xl p-4 border border-white/10">
          ...
          <div class="text-xs text-indigo-400 mt-0.5">${c} Finalized</div>
        </div>
        ...
        <div class="glass rounded-xl p-4 border border-white/10">
          ...
          <div class="text-xs text-emerald-400 mt-0.5">${l} Total Electors</div>
        </div>
  ...
  `,Rt(e,t,r,i,a,o)}function Rt(e,t,n,r,i,a){let o=e.querySelector(`#adminTabPosters`),s=e.querySelector(`#adminTabNotices`),c=e.querySelector(`#adminTabIndex`),l=e.querySelector(`#adminPanelPosters`),u=e.querySelector(`#adminPanelNotices`),d=e.querySelector(`#adminPanelIndex`),f=(e,t)=>{[o,s,c].forEach(e=>{e.classList.remove(`bg-indigo-600`,`text-white`,`shadow-lg`),e.classList.add(`text-slate-400`)}),[l,u,d].forEach(e=>e.classList.add(`hidden`)),e.classList.add(`bg-indigo-600`,`text-white`,`shadow-lg`),e.classList.remove(`text-slate-400`),t.classList.remove(`hidden`)};o?.addEventListener(`click`,()=>f(o,l)),s?.addEventListener(`click`,()=>f(s,u)),c?.addEventListener(`click`,()=>f(c,d)),e.querySelector(`#btnPrintAllDoorPosters`)?.addEventListener(`click`,()=>{if(!a.length){alert(`No polling booths configured to print.`);return}ae(a,n,r)}),e.querySelector(`#btnPrintMasterDirectory`)?.addEventListener(`click`,()=>{if(!a.length){alert(`No polling booths configured to print.`);return}oe(a,n,r)}),e.querySelectorAll(`.print-single-booth-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=Number(e.dataset.num),i=a.find(e=>Number(e.boothNumber)===t);i&&ie(i,n,r)})}),e.querySelectorAll(`.print-notice-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.id,r=i.find(e=>String(e.id)===String(t));r&&re(r,n)})}),e.querySelector(`#classIndexSearch`)?.addEventListener(`input`,t=>{let n=(t.target.value||``).toLowerCase().trim(),r=0;e.querySelectorAll(`.class-index-card`).forEach(e=>{let t=(e.dataset.name||``).toLowerCase(),i=(e.dataset.dept||``).toLowerCase(),a=(e.dataset.booth||``).toLowerCase(),o=!n||t.includes(n)||i.includes(n)||a.includes(n);e.style.display=o?``:`none`,o&&r++});let i=e.querySelector(`#classIndexNoResults`);i&&i.classList.toggle(`hidden`,r>0)}),e.querySelector(`#btnLoadTemplates`)?.addEventListener(`click`,async()=>{let o=Pt(n,r,a);if(confirm(`Load ${o.length} standard statutory notices (Election Notification, Booth Allotments, Code of Conduct, Voter Guidelines, Counting Notice)? Any existing notices with same IDs will be updated.`))try{I(e.querySelector(`#btnLoadTemplates`),!0,`Loading...`);let n=new Map(i.map(e=>[e.id,e]));o.forEach(e=>{n.set(e.id,e)});let r=Array.from(n.values());await C.adminSaveNotices(t,r),L(`Statutory templates loaded successfully!`,`success`),await It(e,t)}catch(t){L(`Error loading templates: `+t.message,`error`),I(e.querySelector(`#btnLoadTemplates`),!1,`⚡ Load Statutory Templates`)}}),e.querySelectorAll(`.toggle-notice-btn`).forEach(n=>{n.addEventListener(`click`,async()=>{let r=n.dataset.id,a=i.find(e=>String(e.id)===String(r));if(!a)return;let o=!(a.isPublished!==!1&&a.isPublished!==`false`);a.isPublished=o;try{await C.adminSaveNotice(t,a),L(`Notice ${o?`published`:`unpublished`}.`,`info`),await It(e,t)}catch(e){L(`Error updating status: `+e.message,`error`)}})}),e.querySelectorAll(`.delete-notice-btn`).forEach(n=>{n.addEventListener(`click`,async()=>{let r=n.dataset.id,a=i.find(e=>String(e.id)===String(r));if(a&&confirm(`Delete notice "${a.title}"? This cannot be undone.`))try{await C.adminDeleteNotice(t,r),L(`Notice deleted.`,`info`),await It(e,t)}catch(e){L(`Error deleting notice: `+e.message,`error`)}})});let p=e.querySelector(`#noticeEditModal`),m=e.querySelector(`#noticeEditOverlay`),h=e.querySelector(`#editModalCloseBtn`),g=e.querySelector(`#btnCancelNoticeEdit`),_=e.querySelector(`#formNoticeEdit`),v=(t=null)=>{e.querySelector(`#editModalHeading`).textContent=t?`Edit Official Notice`:`Draft New Official Notice`,e.querySelector(`#editNoticeId`).value=t?t.id:``,e.querySelector(`#editNoticeTitle`).value=t?t.title:``,e.querySelector(`#editNoticeRef`).value=t?t.refNo||``:`${n.collegeShortName||`GVC`}/ELEC/${n.electionYear||2026}/NOTIF-${String(i.length+1).padStart(2,`0`)}`,e.querySelector(`#editNoticeDate`).value=t?t.date||``:new Date().toISOString().split(`T`)[0],e.querySelector(`#editNoticeCategory`).value=t&&t.category||`Statutory Notification`,e.querySelector(`#editNoticeSignatory`).value=t?t.signatoryName||``:`Returning Officer`,e.querySelector(`#editNoticeSignTitle`).value=t?t.signatoryTitle||``:`Returning Officer, ${n.collegeName||`Government Victoria College`}`,e.querySelector(`#editNoticeContent`).value=t&&t.content||``,e.querySelector(`#editNoticePublished`).checked=t?t.isPublished!==!1&&t.isPublished!==`false`:!0,e.querySelector(`#editNoticePinned`).checked=t?!!t.pinned:!1,p.classList.remove(`hidden`),e.querySelector(`#editNoticeTitle`).focus()},y=()=>{p.classList.add(`hidden`),_.reset()};e.querySelector(`#btnDraftNewNotice`)?.addEventListener(`click`,()=>v(null)),e.querySelector(`#btnCreateNoticeSecondary`)?.addEventListener(`click`,()=>v(null)),e.querySelectorAll(`.edit-notice-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.id,n=i.find(e=>String(e.id)===String(t));n&&v(n)})}),m?.addEventListener(`click`,y),h?.addEventListener(`click`,y),g?.addEventListener(`click`,y),_?.addEventListener(`submit`,async n=>{n.preventDefault();let r=e.querySelector(`#editNoticeId`).value.trim(),i=e.querySelector(`#editNoticeTitle`).value.trim(),a=e.querySelector(`#editNoticeRef`).value.trim(),o=e.querySelector(`#editNoticeDate`).value.trim(),s=e.querySelector(`#editNoticeCategory`).value.trim(),c=e.querySelector(`#editNoticeSignatory`).value.trim(),l=e.querySelector(`#editNoticeSignTitle`).value.trim(),u=e.querySelector(`#editNoticeContent`).value.trim(),d=e.querySelector(`#editNoticePublished`).checked,f=e.querySelector(`#editNoticePinned`).checked;if(!i||!u){alert(`Title and notice content are required.`);return}let p={id:r||`notice_`+Date.now()+`_`+Math.random().toString(36).substr(2,4),title:i,refNo:a,date:o,category:s,signatoryName:c,signatoryTitle:l,content:u,isPublished:d,pinned:f};try{I(e.querySelector(`#btnSaveNoticeSubmit`),!0,`Saving...`),await C.adminSaveNotice(t,p),L(`Notice saved successfully!`,`success`),y(),await It(e,t)}catch(t){L(`Error saving notice: `+t.message,`error`),I(e.querySelector(`#btnSaveNoticeSubmit`),!1,`Save Notice`)}})}var zt=document.getElementById(`app`);document.body.insertAdjacentHTML(`afterbegin`,`
