                      ...
                      ${e.candidates.map((r,i)=>{let a=i<t&&r.votes>0,o=a?r.votes-n:0;return`
                          <tr class="${a?`bg-white/[0.02]`:``}">
                            ...
                            <td class="py-4 text-center">
                              ${a?d?`<span class="text-emerald-400 text-[10px] font-black border border-emerald-400/30 px-2 py-0.5 rounded bg-emerald-500/10 tracking-wider">ELECTED</span>`:`<span class="text-amber-400 text-[10px] font-black border border-amber-400/30 px-2 py-0.5 rounded bg-amber-500/10 tracking-wider">LEADING</span>`:``}
                            </td>
                          </tr>
                        `}).join(``)}
                    </tbody>
            ...
            ${i.map((e,t)=>{let n=f>0?(e.votes/f*100).toFixed(1):0,r=u>0?e.votes/u*100:0,i=t<l&&e.votes>0,a=i?e.votes-p:0,o=sets?.resultsLocked===`true`;return`
                <div class="relative">
                  <div class="flex justify-between items-end mb-2 relative z-10">
                    <div class="flex items-center gap-3">
                      <div class="w-8 h-8 rounded-full ${i?o?`bg-emerald-500 text-emerald-950`:`bg-amber-500 text-amber-950`:`bg-white/10 text-white`} flex items-center justify-center font-bold text-sm shadow-lg">
                        ${i?o?`🏆`:`★`:t+1}
                      </div>
                      <div>
                        <div class="flex items-center gap-2">
                  ...
                  <div class="h-4 w-full bg-slate-800 rounded-full overflow-hidden relative">
                    <div class="h-full rounded-full transition-all duration-1000 ease-out ${i?o?`bg-gradient-to-r from-emerald-400 to-emerald-600`:`bg-gradient-to-r from-amber-400 to-amber-600`:`bg-gradient-to-r from-indigo-500 to-purple-600`}" style="width: ${r}%"></div>
                  </div>
                </div>
              `}).join(``)}
...
`).filter(e=>e.trim());if(n.length<2){A(`CSV file appears empty.`,`error`);return}let r=-1;for(let e=0;e<Math.min(n.length,20);e++){let t=n[e].split(`,`).map(e=>e.trim().replace(/^"|"$/g,``));if(f.every(e=>t.includes(e))){r=e,d=f;break}if(p.every(e=>t.includes(e))){r=e,d=p;break}}if(r===-1){A(`Missing required columns. Please use one of the standard templates.`,`error`),e.querySelector(`#csvPreview`).classList.add(`hidden`);return}let i=n[r].split(`,`).map(e=>e.trim().replace(/^"|"$/g,``)),a=d.map(e=>i.indexOf(e));u=n.slice(r+1).map(e=>{let t=[],n=``,r=!1;for(let i of e+`,`)i===`"`?r=!r:i===`,`&&!r?(t.push(n.trim()),n=``):n+=i;return a.map(e=>t[e]??``)}).filter(e=>e[1]&&e[1].trim()!==``&&e[1]!==`NAME`),d.indexOf(`NAME`);let o=d.indexOf(`Dept`),s=[];s=d===f?[...new Set(u.map(e=>e[d.indexOf(`CLASS`)]))].sort():[...new Set(u.map(e=>`${e[d.indexOf(`YEAR`)]} ${e[d.indexOf(`STREAM`)]} ${e[o]}`.trim()))].sort();let c=[...new Set(u.map(e=>e[o]))].sort();e.querySelector(`#csvSummary`).innerHTML=`
  ...
  `);try{let n=await C.getPublicSchedule();gt(e.querySelector(`#adminMain`),t,n)}catch(t){e.querySelector(`#adminMain`).innerHTML=`<div class="alert alert-error">❌ ${O(t.message)}</div>`}}}function gt(e,t,n){let r=e=>{if(!e)return``;let t=new Date(e);if(isNaN(t.getTime()))return``;let n=e=>String(e).padStart(2,`0`);return`${t.getFullYear()}-${n(t.getMonth()+1)}-${n(t.getDate())}T${n(t.getHours())}:${n(t.getMinutes())}`};e.innerHTML=`
