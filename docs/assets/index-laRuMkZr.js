...
- **THE UNIVERSITY UNION COUNCILLOR (2 Posts)**`,h=d.length>0?d.map(p).join(`
`):`- **I UG REPRESENTATIVE**
- **II UG REPRESENTATIVE**
- **III UG REPRESENTATIVE**
- **PG REPRESENTATIVE**`,g=u.length>0?u.map(p).join(`
...
**Returning Officer**  
*(College Seal)*`},{id:`statutory_notice_booth_allotment`,title:`Notice on Polling Booths, Station Locations & Class Allotments`,refNo:`${s}/ELEC/${a}/NOTIF-02`,date:c,category:`Polling Booth Info`,pinned:!0,isPublished:!0,signatoryName:`Returning Officer`,signatoryTitle:`Returning Officer, ${o}`,content:`### POLLING BOOTHS & STATIONS ALLOTMENT NOTICE

Notice is hereby given to all students and electors of **${o}** regarding the designated **Polling Stations and Booth Allotments** for the College Union Elections ${a}.

Voting shall take place strictly at the designated polling booths between the official polling hours:
**${_(t.pollingStart,`9:30 AM`)} to ${_(t.pollingEnd,`1:30 PM`)}**.
...
5. **Silence Period**:
   - All physical canvassing and campaigning shall cease 24 hours prior to the commencement of polling.`},{id:`statutory_notice_voter_instructions`,title:`Voter Guidelines & Mandatory Identity Verification Instructions`,refNo:`${s}/ELEC/${a}/VOTE-04`,date:c,category:`Voter Instructions`,pinned:!1,isPublished:!0,signatoryName:`Returning Officer`,signatoryTitle:`Returning Officer, ${o}`,content:`### GUIDELINES FOR VOTERS ON POLLING DAY

All bona fide regular students whose names appear in the Final Nominal Roll of **${o}** are requested to note the following crucial voting guidelines:
...
6. Vacate the polling station premises immediately to maintain smooth queue movement.`},{id:`statutory_notice_counting_and_results`,title:`Notice on Scrutiny, Counting of Ballots & Results Declaration`,refNo:`${s}/ELEC/${a}/COUNT-05`,date:c,category:`Counting & Results`,pinned:!1,isPublished:!0,signatoryName:`Returning Officer`,signatoryTitle:`Returning Officer, ${o}`,content:`### COUNTING OF VOTES AND DECLARATION OF RESULTS
...
- **Counting Commences**: ${_(t.resultsStart,`Immediately following closure of poll`)}
...
5. In the event of an equality of votes between two candidates, the result shall be decided by the Returning Officer drawing lots in the presence of the candidates.`}]}async function Ft(e){let t=U();t&&(W(e,`notices`,`
  ...
  `,Rt(e,t,r,i,a,o,c)}function Rt(e,t,n,r,i,a,o=[]){let s=e.querySelector(`#adminTabPosters`),c=e.querySelector(`#adminTabNotices`),l=e.querySelector(`#adminTabIndex`),u=e.querySelector(`#adminPanelPosters`),d=e.querySelector(`#adminPanelNotices`),f=e.querySelector(`#adminPanelIndex`),p=(e,t)=>{[s,c,l].forEach(e=>{e.classList.remove(`bg-indigo-600`,`text-white`,`shadow-lg`),e.classList.add(`text-slate-400`)}),[u,d,f].forEach(e=>e.classList.add(`hidden`)),e.classList.add(`bg-indigo-600`,`text-white`,`shadow-lg`),e.classList.remove(`text-slate-400`),t.classList.remove(`hidden`)};s?.addEventListener(`click`,()=>p(s,u)),c?.addEventListener(`click`,()=>p(c,d)),l?.addEventListener(`click`,()=>p(l,f)),e.querySelector(`#btnPrintAllDoorPosters`)?.addEventListener(`click`,()=>{if(!a.length){alert(`No polling booths configured to print.`);return}ae(a,n,r)}),e.querySelector(`#btnPrintMasterDirectory`)?.addEventListener(`click`,()=>{if(!a.length){alert(`No polling booths configured to print.`);return}oe(a,n,r)}),e.querySelectorAll(`.print-single-booth-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=Number(e.dataset.num),i=a.find(e=>Number(e.boothNumber)===t);i&&ie(i,n,r)})}),e.querySelectorAll(`.print-notice-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.id,r=i.find(e=>String(e.id)===String(t));r&&re(r,n)})}),e.querySelector(`#classIndexSearch`)?.addEventListener(`input`,t=>{let n=(t.target.value||``).toLowerCase().trim(),r=0;e.querySelectorAll(`.class-index-card`).forEach(e=>{let t=(e.dataset.name||``).toLowerCase(),i=(e.dataset.dept||``).toLowerCase(),a=(e.dataset.booth||``).toLowerCase(),o=!n||t.includes(n)||i.includes(n)||a.includes(n);e.style.display=o?``:`none`,o&&r++});let i=e.querySelector(`#classIndexNoResults`);i&&i.classList.toggle(`hidden`,r>0)}),e.querySelector(`#btnLoadTemplates`)?.addEventListener(`click`,async()=>{let s=Pt(n,r,a,o);if(confirm(`Load ${s.length} standard statutory notices (Election Notification, Booth Allotments, Code of Conduct, Voter Guidelines, Counting Notice)? Any existing notices with same IDs will be updated.`))try{I(e.querySelector(`#btnLoadTemplates`),!0,`Loading...`);let n=new Map(i.map(e=>[e.id,e]));s.forEach(e=>{n.set(e.id,e)});let r=Array.from(n.values());await C.adminSaveNotices(t,r),L(`Statutory templates loaded successfully!`,`success`),await It(e,t)}catch(t){L(`Error loading templates: `+t.message,`error`),I(e.querySelector(`#btnLoadTemplates`),!1,`⚡ Load Statutory Templates`)}}),e.querySelectorAll(`.toggle-notice-btn`).forEach(n=>{n.addEventListener(`click`,async()=>{let r=n.dataset.id,a=i.find(e=>String(e.id)===String(r));if(!a)return;let o=!(a.isPublished!==!1&&a.isPublished!==`false`);a.isPublished=o;try{await C.adminSaveNotice(t,a),L(`Notice ${o?`published`:`unpublished`}.`,`info`),await It(e,t)}catch(e){L(`Error updating status: `+e.message,`error`)}})}),e.querySelectorAll(`.delete-notice-btn`).forEach(n=>{n.addEventListener(`click`,async()=>{let r=n.dataset.id,a=i.find(e=>String(e.id)===String(r));if(a&&confirm(`Delete notice "${a.title}"? This cannot be undone.`))try{await C.adminDeleteNotice(t,r),L(`Notice deleted.`,`info`),await It(e,t)}catch(e){L(`Error deleting notice: `+e.message,`error`)}})});let m=e.querySelector(`#noticeEditModal`),h=e.querySelector(`#noticeEditOverlay`),g=e.querySelector(`#editModalCloseBtn`),_=e.querySelector(`#btnCancelNoticeEdit`),v=e.querySelector(`#formNoticeEdit`),y=(t=null)=>{e.querySelector(`#editModalHeading`).textContent=t?`Edit Official Notice`:`Draft New Official Notice`,e.querySelector(`#editNoticeId`).value=t?t.id:``,e.querySelector(`#editNoticeTitle`).value=t?t.title:``,e.querySelector(`#editNoticeRef`).value=t?t.refNo||``:`${n.collegeShortName||`GVC`}/ELEC/${n.electionYear||2026}/NOTIF-${String(i.length+1).padStart(2,`0`)}`,e.querySelector(`#editNoticeDate`).value=t?t.date||``:new Date().toISOString().split(`T`)[0],e.querySelector(`#editNoticeCategory`).value=t&&t.category||`Statutory Notification`,e.querySelector(`#editNoticeSignatory`).value=t?t.signatoryName||``:`Returning Officer`,e.querySelector(`#editNoticeSignTitle`).value=t?t.signatoryTitle||``:`Returning Officer, ${n.collegeName||`Government Victoria College`}`,e.querySelector(`#editNoticeContent`).value=t&&t.content||``,e.querySelector(`#editNoticePublished`).checked=t?t.isPublished!==!1&&t.isPublished!==`false`:!0,e.querySelector(`#editNoticePinned`).checked=t?!!t.pinned:!1,m.classList.remove(`hidden`),e.querySelector(`#editNoticeTitle`).focus()},b=()=>{m.classList.add(`hidden`),v.reset()};e.querySelector(`#btnDraftNewNotice`)?.addEventListener(`click`,()=>y(null)),e.querySelector(`#btnCreateNoticeSecondary`)?.addEventListener(`click`,()=>y(null)),e.querySelectorAll(`.edit-notice-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.id,n=i.find(e=>String(e.id)===String(t));n&&y(n)})}),h?.addEventListener(`click`,b),g?.addEventListener(`click`,b),_?.addEventListener(`click`,b),v?.addEventListener(`submit`,async n=>{n.preventDefault();let r=e.querySelector(`#editNoticeId`).value.trim(),i=e.querySelector(`#editNoticeTitle`).value.trim(),a=e.querySelector(`#editNoticeRef`).value.trim(),o=e.querySelector(`#editNoticeDate`).value.trim(),s=e.querySelector(`#editNoticeCategory`).value.trim(),c=e.querySelector(`#editNoticeSignatory`).value.trim(),l=e.querySelector(`#editNoticeSignTitle`).value.trim(),u=e.querySelector(`#editNoticeContent`).value.trim(),d=e.querySelector(`#editNoticePublished`).checked,f=e.querySelector(`#editNoticePinned`).checked;if(!i||!u){alert(`Title and notice content are required.`);return}let p={id:r||`notice_`+Date.now()+`_`+Math.random().toString(36).substr(2,4),title:i,refNo:a,date:o,category:s,signatoryName:c,signatoryTitle:l,content:u,isPublished:d,pinned:f};try{I(e.querySelector(`#btnSaveNoticeSubmit`),!0,`Saving...`),await C.adminSaveNotice(t,p),L(`Notice saved successfully!`,`success`),b(),await It(e,t)}catch(t){L(`Error saving notice: `+t.message,`error`),I(e.querySelector(`#btnSaveNoticeSubmit`),!1,`Save Notice`)}})}var zt=document.getElementById(`app`);document.body.insertAdjacentHTML(`afterbegin`,`
