...
5. In the event of an equality of votes between two candidates, the result shall be decided by the Returning Officer drawing lots in the presence of the candidates.`}]}async function jt(e){let t=H();t&&(U(e,`notices`,`
    <div class="text-center py-16">
      <span class="spinner" style="width:2.5rem;height:2.5rem;border-width:4px;"></span>
      <p class="text-slate-400 mt-4 text-sm">Loading notices and polling booth poster center...</p>
    </div>
  `),await Mt(e.querySelector(`#adminMain`),t))}async function Mt(e,t){if(e)try{let[n,r]=await Promise.all([C.adminGetNotices(t,!0).catch(()=>({})),C.getNominalRoll().catch(()=>[])]),i=n.settings||{},a=n.schedule||{},o=Array.isArray(n.notices)?n.notices:[],s=Array.isArray(n.booths)?n.booths:[];Array.isArray(n.locations)&&n.locations;let c={};r.forEach(e=>{let t=String(e.CLASS||``).trim(),n=String(e.Dept||``).trim(),r=t.toUpperCase().includes(`RESEARCH`)||t.toUpperCase().includes(`SCHOLAR`)||t.toUpperCase().includes(`PHD`)?`RESEARCH SCHOLAR - ${n}`:t;r&&(c[r]||(c[r]={name:r,dept:n,count:0}),c[r].count++)}),s.forEach(e=>{let t=Array.isArray(e.classes)?e.classes:[];e.totalStudents=0,t.forEach(t=>{c[t]&&(e.totalStudents+=c[t].count)})}),Nt(e,t,i,a,o,s,c)}catch(t){console.error(`Error loading admin notices:`,t),e.innerHTML=`<div class="alert alert-error">❌ ${F(t.message||`Failed to load notices`)}</div>`}}function Nt(e,t,r,i,a,o,s){r.collegeName||n.COLLEGE_NAME,r.collegeShortName||n.COLLEGE_SHORT_NAME,r.electionYear||new Date().getFullYear();let c=a.filter(e=>e.isPublished!==!1&&e.isPublished!==`false`).length,l=o.reduce((e,t)=>e+(t.totalStudents||0),0);e.innerHTML=`
    <div class="page-enter space-y-6">
...
`);var It=document.getElementById(`sync-status`),Lt=document.getElementById(`sync-icon`),Rt=document.getElementById(`sync-text`);m(e=>{e===`saving`?(It.classList.remove(`translate-y-[-150%]`,`opacity-0`),It.classList.add(`translate-y-0`,`opacity-100`),Lt.className=`animate-spin inline-block text-indigo-400`,Lt.innerHTML=`&#8635;`,Rt.innerText=`Saving changes...`):e===`saved`?(Lt.className=`inline-block text-emerald-400`,Lt.innerHTML=`&#10003;`,Rt.innerText=`All changes saved`):e===`idle`&&(It.classList.remove(`translate-y-0`,`opacity-100`),It.classList.add(`translate-y-[-150%]`,`opacity-0`))}),C.initPublicData();