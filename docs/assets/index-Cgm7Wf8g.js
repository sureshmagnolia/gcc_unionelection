...
Are you absolutely sure?`))return;E(t,!0,`🗑️ Wiping...`);let i=n.querySelector(`#wipeStatus`);i.innerHTML=``;try{await C.adminWipeData(r),n.querySelector(`#wipePasswordInput`).value=``,i.innerHTML=`
  ...
  `,e.querySelector(`#btnPrintOfficial`).addEventListener(`click`,()=>{let e=`
    ...
    `),t.document.close()});let h=e.querySelector(`#btnToggleLock`);h&&(h.onclick=async()=>{if(confirm(d?`Unlock election results? Vote entries will be re-enabled.`:`Lock and freeze election results? No further vote entries will be allowed.`)){h.disabled=!0,h.textContent=`Please wait...`;try{d=(await C.adminToggleLockResults(t)).locked,o.resultsLocked=d?`true`:`false`,D(d?`🔒 Results locked and frozen.`:`🔓 Results unlocked for editing.`,`success`),J(e,t,n,r,i,a,o,s,c)}catch(e){D(e.message,`error`),h.disabled=!1,h.textContent=d?`🔓 Unlock Results`:`🔒 Freeze / Lock`}}});let g=e.querySelector(`#btnTogglePublic`);g&&(g.onclick=async()=>{if(confirm(f?`Hide election results from public view? The public portal will show counting in progress.`:`Publish election results to the public portal? Anyone visiting the site will see live results.`)){g.disabled=!0,g.textContent=`Please wait...`;try{f=(await C.adminTogglePublishResults(t)).published,o.resultsPublished=f?`true`:`false`,D(f?`📢 Results published to public portal!`:`👁️‍🗨️ Results hidden from public view.`,`success`),J(e,t,n,r,i,a,o,s,c)}catch(e){D(e.message,`error`),g.disabled=!1,g.textContent=f?`👁️‍🗨️ Hide Public View`:`📢 Publish to Public`}}})}var $e=`election_results_cache`,Y=`election_results_last_fetch`,et=300*1e3;async function tt(e){e.innerHTML=`
    <div class="page-enter min-h-screen">
      <header class="no-print sticky top-0 z-10 border-b border-white/10 glass">
        <div class="max-w-5xl mx-auto px-6 py-3 flex items-center justify-between">
          <div class="flex items-center gap-4">
