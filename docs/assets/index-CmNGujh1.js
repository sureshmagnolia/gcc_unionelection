...
This will remove the final candidate list from public view.`)){E(n,!0,`🚫 Unpublish`);try{await C.adminUnpublishFinalList(a),D(`Final list unpublished.`,`success`),V(e,{validListPublished:`true`,finalListPublished:`false`},r,i,a)}catch(e){D(`Failed: ${e.message}`,`error`),E(n,!1,`🚫 Unpublish`)}}}),e.querySelector(`#toggleResultsPublishBtn`)?.addEventListener(`click`,async n=>{let o=n.currentTarget,s=t.resultsPublished===`true`;if(confirm(s?`Hide election results from public view? Public visitors will see counting in progress.`:`Publish election results to the public portal? Results will be immediately visible to all visitors.`)){E(o,!0,`Updating...`);try{let n=await C.adminTogglePublishResults(a);t.resultsPublished=n.published?`true`:`false`,D(n.published?`📢 Results published to public view!`:`👁️‍🗨️ Results hidden from public view.`,`success`),V(e,t,r,i,a)}catch(e){D(`Failed: ${e.message}`,`error`),E(o,!1,s?`🚫 Hide Results from Public`:`📢 Publish Results to Public`)}}}),e.querySelector(`#toggleResultsLockBtn`)?.addEventListener(`click`,async n=>{let o=n.currentTarget,s=t.resultsLocked===`true`;if(confirm(s?`Unlock election results? Vote entry modifications will be re-enabled.`:`Freeze and lock election results? Further vote entries will be blocked.`)){E(o,!0,`Updating...`);try{let n=await C.adminToggleLockResults(a);t.resultsLocked=n.locked?`true`:`false`,D(n.locked?`🔒 Results locked and frozen.`:`🔓 Results unlocked.`,`success`),V(e,t,r,i,a)}catch(e){D(`Failed: ${e.message}`,`error`),E(o,!1,s?`🔓 Unlock Results`:`🔒 Freeze / Lock Results`)}}})}async function ze(e){let t=L();if(t){R(e,`posts`,`
  ...
  `);try{let[n,r,i,a,o,s,c]=await Promise.all([C.getNominalRoll(),C.adminGetBooths(t).catch(()=>[]),C.adminGetLocations(t).catch(()=>[]),C.adminGetPosts(t).catch(()=>[]),C.getFinalNominations().catch(()=>({active:[]})),C.adminGetBallotPlan(t).catch(()=>null),C.adminGetSettings(t).catch(()=>({}))]);Ue(e.querySelector(`#adminMain`),t,n,r,i,a,o,s,c)}catch(t){e.querySelector(`#adminMain`).innerHTML=`<div class="alert alert-error">❌ ${T(t.message)}</div>`}}}function Ue(e,t,r,i,a,o,s,c,l){let u={};r.forEach(e=>{let t=String(e.CLASS||`Unknown`).trim(),n=String(e.Dept||`Unknown`).trim();u[t]||(u[t]={name:t,dept:n,count:0}),u[t].count++});let d=Object.values(u).sort((e,t)=>e.name.localeCompare(t.name)),f=i.length?[...i]:[{boothNumber:1,roomName:``,classes:[]}],p=[...a],m=!0,h=()=>{f.forEach(e=>e.totalStudents=0);let n=[];d.forEach(e=>{let t=f.find(t=>t.classes.includes(e.name));t?t.totalStudents+=e.count:n.push(e)});let i=window.scrollY;e.innerHTML=`
        <!-- Locations Modal -->
...
Are you absolutely sure?`))return;E(t,!0,`🗑️ Wiping...`);let i=n.querySelector(`#wipeStatus`);i.innerHTML=``;try{await C.adminWipeData(r),n.querySelector(`#wipePasswordInput`).value=``,i.innerHTML=`
        ...
        </div>`,D(`All data wiped successfully.`,`success`)}catch(e){i.innerHTML=`<div class="alert alert-error mt-3">❌ ${T(e.message)}</div>`,D(`Failed: ${e.message}`,`error`)}finally{E(t,!1,`🗑️ Permanently Wipe All Data`)}})}async function Qe(e){let t=L();if(!t)return;R(e,`results`,`
...
`).filter(e=>e.trim());if(n.length<2){D(`CSV file appears empty.`,`error`);return}let r=-1;for(let e=0;e<Math.min(n.length,20);e++){let t=n[e].split(`,`).map(e=>e.trim().replace(/^"|"$/g,``));if(f.every(e=>t.includes(e))){r=e,d=f;break}if(p.every(e=>t.includes(e))){r=e,d=p;break}}if(r===-1){D(`Missing required columns. Please use one of the standard templates.`,`error`),e.querySelector(`#csvPreview`).classList.add(`hidden`);return}let i=n[r].split(`,`).map(e=>e.trim().replace(/^"|"$/g,``)),a=d.map(e=>i.indexOf(e));u=n.slice(r+1).map(e=>{let t=[],n=``,r=!1;for(let i of e+`,`)i===`"`?r=!r:i===`,`&&!r?(t.push(n.trim()),n=``):n+=i;return a.map(e=>t[e]??``)}).filter(e=>e[1]&&e[1].trim()!==``&&e[1]!==`NAME`),d.indexOf(`NAME`);let o=d.indexOf(`Dept`),s=[];s=d===f?[...new Set(u.map(e=>e[d.indexOf(`CLASS`)]))].sort():[...new Set(u.map(e=>`${e[d.indexOf(`YEAR`)]} ${e[d.indexOf(`STREAM`)]} ${e[o]}`.trim()))].sort();let c=[...new Set(u.map(e=>e[o]))].sort();e.querySelector(`#csvSummary`).innerHTML=`
  ...
  `),f.document.close()}async function ct(e){let t=L();if(t){R(e,`schedule`,`
...
`),n.APPS_SCRIPT_URL.includes(`YOUR_SCRIPT_ID`)&&(document.body.insertAdjacentHTML(`afterbegin`,`
  ...
  `),document.getElementById(`app`).style.marginTop=`48px`);var Z=e=>t=>{ft.innerHTML=``,e(ft,t)};o.on(`/`,Z(se)).on(`/submit`,Z(N)).on(`/find`,Z(he)).on(`/valid-list`,Z(ve)).on(`/final-list`,Z(xe)).on(`/withdraw`,Z(we)).on(`/results`,Z(tt)).on(`/nominal-roll`,Z(rt)).on(`/admin`,Z(Me)).on(`/admin/dashboard`,Z(Ne)).on(`/admin/verify`,Z(Pe)).on(`/admin/withdrawals`,Z(Ie)).on(`/admin/publish`,Z(Re)).on(`/admin/posts`,Z(ze)).on(`/admin/ballots`,Z(Xe)).on(`/admin/booths`,Z(He)).on(`/admin/counting`,Z(We)).on(`/admin/results-entry`,Z(Je)).on(`/admin/results`,Z(Qe)).on(`/admin/nominal-roll`,Z(X)).on(`/admin/schedule`,Z(ct)).on(`/admin/direct-nomination`,Z(ut)).on(`/admin/testing`,Z(Ze)).on(`/admin/audit`,Z(dt)).setDefault(`/`),document.addEventListener(`click`,e=>{let t=e.target.closest(`[data-nav]`);t&&(e.preventDefault(),o.navigate(t.dataset.nav))}),o.start(),document.body.insertAdjacentHTML(`beforeend`,`
